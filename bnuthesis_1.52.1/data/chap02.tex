...
本文采用文献\onlinecite{DPTMbyYJQ}中所提出的基于时间序列分析的工作负载预测模型来满足VP-TALk算法预知工作负载率的需求。 大量的模拟实验已经证明， \onlinecite{DPTMbyYJQ}所提出的组合预测模型可以得到平均相对误差低于3\%、归一化方差小于0.5的任务负载预测效果。 具体的负载预测模型构建的公式推导和演化请参见文章\onlinecite{DPTMbyYJQ}的相关章节。这里，主要简单介绍整个预测流程。
